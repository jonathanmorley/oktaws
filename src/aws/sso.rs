...
impl SsoClient {
  pub async fn new(org_id: &str, auth_code: &str) -> Result<Self> {
    #[derive(Deserialize)]
    struct SsoTokenResponse { token: String }
  ...
  pub async fn profiles(&self, app_instance_id: &str) -> Result<Vec<Profile>> {
    let response = reqwest::Client::new()
        .get(format!("{BASE_URL}/instance/appinstance/{app_instance_id}/profiles"))
        .header("x-amz-sso_bearer_token", &self.token)
        .header("x-amz-sso-bearer-token", &self.token)
        .send()
        .await?;
  ...
  pub async fn credentials(&self, account_id: &str, role_name: &str) -> Result<aws_sdk_iam::Credentials> {
    #[derive(Debug, Deserialize)]
    #[serde(rename_all = "camelCase")]
    struct Credentials {
    ...
    let RoleCredentials { role_credentials } = reqwest::Client::new()
        ...
        .query(&[("account_id", account_id), ("role_name", role_name), ("debug", "true")])
        .header("x-amz-sso_bearer_token", &self.token)
        .header("x-amz-sso-bearer-token", &self.token)
        .send()
        .await?
