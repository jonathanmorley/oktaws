...
impl Profile {
    fn try_from_config(profile_config: &ProfileConfig, name: String, default_role: Option<String>, default_duration_seconds: Option<i64>) -> Result<Profile, Error> {
...
impl TryFrom<&Path> for Organization
{
    ...
    fn try_from(path: &Path) -> Result<Self, Self::Error> {
        ...
        let profiles = cfg.profiles
            .iter()
            .map(|(name, profile_config)| Profile::try_from_config(profile_config, name.to_string(), cfg.role.clone(), cfg.duration_seconds))
