...
pub fn extract_saml_response(text: &str) -> Result<SamlResponse> {
    ...
    let form = doc
        ...
        .map_err(|_| anyhow!("No SAML form found"))?;

    let url = form
        .attributes
        .borrow()
        .get("action")
        .ok_or_else(|| anyhow!("No SAML URL found"))?
        .to_owned();
    let saml = form
        .as_node()
        .select_first("input[name='SAMLResponse']")
        .map_err(|_| anyhow!("No SAML Response found"))?
        .attributes
        .borrow()
        .get("value")
        .ok_or_else(|| anyhow!("No SAML response value found"))?
        .to_owned();
