...
impl Client {
    ...
    pub async fn get_saml_response(&self, app_url: Url) -> Result<SamlResponse> {
        ...
        if let Some(token) = Client::extra_verification_token(response.clone())? {
            ...
            self.get_session_token(&LoginRequest::from_state_token(token)).await?;
...
pub fn extract_saml_response(text: &str) -> Result<SamlResponse> {
    ...
    let form = doc.select_first("form[id='appForm']").map_err(|_| anyhow!("No SAML form found"))?;
    
    let url = form.attributes.borrow().get("action").ok_or_else(|| anyhow!("No SAML URL found"))?.to_owned();
    let saml = form.as_node().select_first("input[name='SAMLResponse']").map_err(|_| anyhow!("No SAML Response found"))?.attributes.borrow().get("value").ok_or_else(|| anyhow!("No SAML response value found"))?.to_owned();
    let relay_state = form.as_node().select_first("input[name='RelayState']").ok().and_then(|node| node.attributes.borrow().get("value").map(ToOwned::to_owned));
