...
impl Organization {
    pub fn from_file_path<P>(path: &P) -> Result<Organization, Error>
    where
        P: ?Sized + AsRef<Path>,
    {
        let name = path
        ...
        let base_url = file_toml
            .get("url")
            .and_then(|r| toml_to_string(r))
            .unwrap_or_else(|| format!("https://{}.okta.com", name));
    ...
    pub fn auth_with_credentials(&self) -> Result<AuthenticationTransaction, Error> {
        ...
        match transaction.status() {
            Some(TransactionState::Success) => {
                credentials::save_credentials(&self.base_url, &self.username, &password)?;
                Ok(transaction)
            }
            Some(TransactionState::MfaRequired) => {
                credentials::save_credentials(&self.base_url, &self.username, &password)?;
                self.handle_mfa(&transaction)
            }
            e => bail!("Unsuccessful login with credentials ({:?})", e),
        }
    }
    ...
    pub fn auth_with_state_token(
        ...
        match transaction.status() {
            ...
            e => bail!("Unsuccessful login with state token ({:?})", e),
        }
    }
    ...
    pub fn handle_mfa(
        ...
        let factors = transaction._embedded().unwrap()["factors"]
            .as_array()
            .unwrap()
            .to_owned();
        ...
        let factor_prompt_response = self
            .client
            .authentication_api()
            .auth_verify_factor(factor_id, factor_prompt_request, true, true)
            .wait()
            .map_err(|e| format_err!("{:?}", e))?;
        ...
        let factor_provided_request = AuthVerifyFactorRequest::new()
            .with_state_token(state_token)
            .with_pass_code(mfa_code);

        let factor_provided_response = self
            .client
            .authentication_api()
            .auth_verify_factor(factor_id, factor_provided_request, true, true)
            .wait()
            .map_err(|e| format_err!("{:?}", e))?;
    ...
    pub fn create_session(&mut self, session_token: &str) -> Result<Session, Error> {
        ...
        let session_request =
            CreateSessionRequest::new().with_session_token(session_token.to_owned());
    ...
    pub fn get_saml_response(&mut self, app_url: &str) -> Result<SamlResponse, Error> {
        ...
        match saml::extract_saml_response(response.clone()) {
            Err(ExtractSamlResponseError::NotFound) => {
                debug!(
                    "No SAML found for app {:?}, will attempt re-login",
                    &app_url
                );
